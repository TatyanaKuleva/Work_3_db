...
class DBManager:
    ...
    def get_companies_and_vacancies_count(self):
        ...
        with (self.conn):
            query = (f'SELECT {self.table_one}.name, COUNT({self.table_two}.id) as vacancy_count '
    ...
    def get_all_vacancies(self):
        ...
        with (self.conn):
            query = (f'SELECT {self.table_two}.name, {self.table_one}.name, {self.table_two}.salary_from, {self.table_two}.url_vacancy '
            ...
            self.cur.execute(query)
            data = self.cur.fetchall()
            return [{'vacancy': row[0], 'employer': row[1], 'salary from': row[2], 'url_vacancy': row[3]} for row in data]
    ...
    def get_avg_salary(self):
        ...
        with (self.conn):
            query = (f'SELECT AVG({self.table_two}.salary_from) '
                     f'FROM {self.table_two} ')
    ...
    def get_vacancies_with_higher_salary(self):
        ...
        with (self.conn):
            ...
            self.cur.execute(query)
            data = self.cur.fetchall()
            return [{'id': row[0], 'vacancy': row[1], 'salary from': row[2], 'salary_to': row[3],
                     'url_vacancy': row[4], 'employer_name': row[5]} for row in data]
    ...
    def get_vacancies_with_keyword(self, word_for_search):
        ...
        with (self.conn):
            ...
            query = (f"SELECT {self.table_two}.id, {self.table_two}.name, {self.table_two}.url_vacancy, "
